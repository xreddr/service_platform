...
def cookbook():
    ...
    categories = cur.execute("SELECT * FROM cookbook_category WHERE user_id = ?;", (session['user_id'],)).fetchall()

    cur.close()
    db.close_db()

    return render_template('cookbook/home.html', recipes=recipes, categories=categories)
